...
var utils = {
    ...
    getUrlOptions: function (options) {
        ...
        if (options.get("online") === false) {
            urls.local = utils.getUrl(scheme + "://" + host + ":" + port, options);
    ...
    getPorts: function (options, cb) {
        ...
        utils.getPort(options.get("host"), port, max, cb);
    },
    getPort: function (host, port, max, cb) {
        portScanner.findAPortNotInUse(port, max, host, cb);
    },
    /**
     * @param {String} ua
     * @returns {Object}
